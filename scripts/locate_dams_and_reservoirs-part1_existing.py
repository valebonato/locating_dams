...
for dam_id in range(1, number_of_dams + 1):
    ...
    if rel_dif_catchment_area_this_dam_cell_value < threshold:
        ...
        #extracts the dam ID value from the map for the specific cell where the dam is located. 
        #this_dam_point is a binary map that contains only the current dam, 
        #so pcr.mapmaximum returns the cell with the dam, and pcr.cellvalue extracts the dam ID value from that cell.dam_id_value = pcr.cellvalue(pcr.mapmaximum(this_dam_point), 1)[0]
        # Make a point map of this dam by assigning the Boolean value 1.0 to the cell where the dam_ids map matches the specified dam_id
        print(dam_id)
